...
class VarNode(Node):
    ...
    def check(self):
        return order.getindval()[self.value]
...
class FuncNode(Node):
    def __init__(self,name,argument,block,proc):
        self.name = name
        self.argument = argument
        self.block = block
        self.proc = proc
    
    def outval(self,valg):
        dict = {}
        for i in range(len(valg)):
            dict[self.argument.ing(i).value] = valg[i].check()
        order.takeval(dict)
        self.block.check()
        x = self.proc.check()
        dict = order.removeval()
        return x
    
    def check(self):
        order.getindval()[self.name.value] = self  
...
class VarInd(Node):
    ...
    def check(self):
        ...
        if i is not None:
            i = i.check()
        if i is None:
            order.getindval()[id] = value.check()
        else:
            if isinstance(id,Node):
                id.check()[i] = value.check()
            else:
                order.getindval()[id][i] = value.check()
...
class Order(Node):
    def __init__(self):
        self.order = [{}]
    def takeval(self,val):
        self.order.append(val)
    def removeval(self):
        return self.order.pop()
    def getindval(self,index = -1):
...
class Func1Node(Node):
    def __init__(self,name,argument):
        self.name = name
        self.argument = argument
    
    def check(self):
        #print(order.getindval(0))
        #print(order.getindval(0)[self.name.value])
        c = order.getindval
        a = c(0)[self.name.value]
        return a.outval(self.argument.value)
